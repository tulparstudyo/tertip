import { resolveAuthorFields } from './author-citation.js';

/**
 * İstanbul Üniversitesi Tez Hazırlama Yönergesi — Madde 20, 21 ve Örnek 3
 * Referans dipnotları: ilk atıf tam bibliyografik bilgi; tekrarlarda kısaltma (a.e. / soyad, eser adı, s. …)
 */

/** Özel dipnot */
export function isCustomFootnote(attrs) {
  if (!attrs) return false;
  if (attrs.isCustom === true) return true;
  if (attrs.citationType === 'custom') return true;
  return false;
}

/** Dipnot kontrolünden hariç tutulacak dipnotlar (yalnızca özel dipnotlar) */
export function isExcludedFromFootnoteCheck(attrs) {
  return isCustomFootnote(attrs);
}
...
/** Dipnotta yazar: iç kapaktaki gibi Ad Soyad (Madde 20/1) */
export function formatAuthorFootnote(source) {
...
/** Süreli yayın cilt no — büyük Romen rakamı (Madde 20/14d, Örnek 3) */
...
/**
 * Tekrar atıf — Madde 21 + Örnek 3
 * consecutive: hemen önceki dipnot aynı kaynak → a.e., s. …
 * repeat: araya başka dipnot girmiş → Soyad, Eser adı, s. …
 */
export function formatTurkishFootnoteShort(source, pageNumber, repeatMode = 'repeat') {
  const pageSuffix = formatPageSuffix(pageNumber);

  if (repeatMode === 'consecutive') {
    return `a.e.${pageSuffix}.`;
  }

  const { lastName } = resolveAuthorFields(source);
  const title = source.title?.trim() || 'a.g.e.';

  if (lastName) {
    return `${lastName}, ${title}${pageSuffix}.`;
  }

  return `${title}${pageSuffix}.`;
}

export function formatTurkishFootnote(
  source,
  pageNumber,
  { repeatMode = 'none' } = {},
) {
  if (repeatMode === 'consecutive' || repeatMode === 'repeat') {
    return formatTurkishFootnoteShort(source, pageNumber, repeatMode);
  }
  return formatTurkishFootnoteFull(source, pageNumber);
}

/** Belge sırasına göre dipnot atıf metinlerini yönergeye uygun yeniden üretir */
export function buildFootnoteCitationUpdates(footnotes, sourceMap) {
  const seenSources = new Set();
  let previousSourceId = null;

  return footnotes.map(({ pos, attrs }) => {
    if (isExcludedFromFootnoteCheck(attrs)) {
      return { pos, attrs, skip: true, reason: 'custom' };
    }

    const sourceId = Number(attrs.sourceId);
    const source = sourceMap.get(sourceId);
    if (!source) {
      return { pos, attrs, skip: true, reason: 'missingSource' };
    }
    ...
    const formattedText = formatTurkishFootnote(source, attrs.pageNumber, { repeatMode });
    seenSources.add(sourceId);
    previousSourceId = sourceId;

    return {
      pos,
      attrs: { ...attrs, formattedText },
      skip: false,
      changed: formattedText !== attrs.formattedText,
    };
  });
}
